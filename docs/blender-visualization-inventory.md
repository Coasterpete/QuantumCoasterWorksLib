...
| `TrainPoseExportV1` JSON | `Quantum.IO` train pose export path and debug commands. | Candidate input if Blender needs full body/bogie/wheel hierarchy instead of the flatter snapshot box layer. | Candidate | Should remain a snapshot contract, not a Blender scene format. |
...
|---|---|---|---|
| `TrainPoseExportV1` Blender importer | Draw full train body, bogie, wheel, and articulated frame hierarchy for pose debugging. | Candidate | Consume existing `quantum.train_pose` v1 JSON without changing the contract. |
