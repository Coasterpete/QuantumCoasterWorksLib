...
- The SVG previews and generated gallery remain technical debug surfaces. They now include top-down X/Z and elevation/profile panels to make hills, drops, and climbs easier to inspect without adding renderer or frontend dependencies. Raw exported centerline samples are still represented, while Catmull-Rom smooth-preview paths are visual-only approximations and do not change JSON, spline, track, or physics behavior.
