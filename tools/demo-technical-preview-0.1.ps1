...
function New-DebugViewportGallery {
    ...
    [void] $builder.AppendLine("    body { margin: 24px; font-family: Segoe UI, Arial, sans-serif; color: #111827; background: #f8fafc; }")
    [void] $builder.AppendLine("    h1 { margin: 0 0 8px; font-size: 24px; }")
    [void] $builder.AppendLine("    p { margin: 0 0 18px; color: #475569; }")
    [void] $builder.AppendLine("    .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 16px; }")
    [void] $builder.AppendLine("    figure { margin: 0; padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; background: #ffffff; }")
    [void] $builder.AppendLine("    h2 { margin: 0 0 10px; font-size: 16px; }")
    ...
    [void] $builder.AppendLine("  <p>Generated technical debug previews from renderer-neutral backend snapshots. This is not a finished editor, viewer, or renderer.</p>")
