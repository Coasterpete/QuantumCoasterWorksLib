...
- `dotnet run --project Quantum.Debug -- distance-inspection-browser [outputHtmlPath]`: write a self-contained static HTML preview for the deterministic `DistanceInspectionSnapshotV1` sample under `artifacts/track/`.
- `dotnet run --project Quantum.Debug -- distance-inspection-browser <inputJsonPath> <outputHtmlPath>`: render an existing `DistanceInspectionSnapshotV1` JSON artifact to self-contained static HTML.
