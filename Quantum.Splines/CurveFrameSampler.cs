...
{
    /// <summary>
    /// Samples generic curve frames without introducing coaster-domain semantics.
    /// </summary>
    public sealed class CurveFrameSampler
    {
        private readonly IArcLengthCurve _curve;
        private readonly Vector3d _referenceNormal;

        public CurveFrameSampler(IArcLengthCurve curve)
            : this(curve, Vector3d.UnitY)
        {
        }

        public CurveFrameSampler(IArcLengthCurve curve, Vector3d referenceNormal)
        {
            _curve = curve ?? throw new ArgumentNullException(nameof(curve));
            _referenceNormal = referenceNormal;
        }

        public CurveFrame GetFrameAt(double s)
        {
        ...
        public static CurveFrame GetFrameAt(IArcLengthCurve curve, double s)
        {
            var sampler = new CurveFrameSampler(curve, Vector3d.UnitY);
            return sampler.GetFrameAt(s);
        }

        public static CurveFrame SampleFrameByLength(
            IArcLengthCurve curve,
            double s,
            Vector3d referenceUp)
        {
            var sampler = new CurveFrameSampler(curve, referenceUp);
            return sampler.GetFrameAt(s);
        }

        public static List<CurveFrame> SampleFramesUniform(
            IArcLengthCurve curve,
            double stepLength,
            Vector3d referenceUp)
        {
            ...
            var frames = new List<CurveFrame>();
        ...
        private static CurveFrame BuildFrame(
            IArcLengthCurve curve,
            double s,
            Vector3d position,
            Vector3d tangent,
            Vector3d referenceNormal)
        {
            ...
            return new CurveFrame(s, position, tangent, normal, binormal);
        }
        ...
        {
            return curve.Length <= MathUtil.Epsilon
                ? 0.0
                : MathUtil.Clamp(s, 0.0, curve.Length);
        }
        ...
        private static Vector3d ResolveFallbackNormal(Vector3d referenceNormal, Vector3d tangent)
        {
            ...
            return NormalizeOrThrow(RejectAlong(fallbackAxis, tangent), "normal");
        }
