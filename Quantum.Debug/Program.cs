...
namespace Quantum.Debug
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            if (command == DebugCommandKind.DebugViewportSnapshotV1Svg)
            {
                if (args.Length < 2 || args.Length > 3)
                {
                    Console.WriteLine("Usage: debug-viewport-snapshot-v1-svg <snapshotJsonPath> [outputSvgPath]");
                    return 1;
                }

                string snapshotJsonPath = args[1];
                string? outputSvgPath = args.Length == 3 ? args[2] : null;
            ...
            if (command == DebugCommandKind.DebugViewportSnapshotV1Browser)
            {
                if (args.Length > 3)
                {
                    Console.WriteLine("Usage: debug-viewport-snapshot-v1-browser [artifactDirectory] [outputHtmlPath]");
                    return 1;
                }

                string? artifactDirectory = args.Length >= 2 ? args[1] : null;
                string? outputHtmlPath = args.Length == 3 ? args[2] : null;
                return DebugViewportSnapshotBrowserCommand.Run(artifactDirectory, outputHtmlPath);
            }

            if (command == DebugCommandKind.DebugViewportSnapshotV1BankingProfile)
            {
                if (args.Length > 2)
                {
                    Console.WriteLine("Usage: debug-viewport-snapshot-v1-banking-profile [outputPath]");
                    return 1;
                }
            ...
            if (command == DebugCommandKind.ContinuousRollDiagnosticsSample)
            {
                if (args.Length > 2)
                {
                    Console.WriteLine("Usage: continuous-roll-diagnostics-sample [outputPath]");
                    return 1;
                }
